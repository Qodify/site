<script>
  ...
  onMount(async () => {
    ...
    const res = await fetch(apiBaseUrl + "/blogposts/" + id[id.length - 1]);
    post = await res.json();
  });
</script>
...
<div  class="container mx-0">
  ...
  {#if post}
    <div in:fade class="col-11 mx-auto my-5">
      <h5>{@html marked(post.title)}</h5>
      <h6 class="mb-2 text-muted">
        {new Date(post.timestamp).toLocaleDateString(undefined, { year: "numeric", month: "long", day: "numeric" })}
      </h6>
<!-- <Prism language="html" code={post.body}/> -->
      <span>{@html marked(post.body)}</span>
    </div>
  {/if}
</div>
